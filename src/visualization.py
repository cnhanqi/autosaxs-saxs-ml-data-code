...
def train_best_model(df, target_col='y1', model_kind='auto'):
    """
    ...
    # Prefer XGBoost when available; otherwise fall back to RandomForest unless
    # a specific model is requested.
    try:
        from sklearn.ensemble import GradientBoostingRegressor, RandomForestRegressor
    except Exception as e:
        raise ImportError(
            "Need scikit-learn for heatmap model training. "
            "Install with: pip install -r requirements.txt"
        ) from e

    model_key = str(model_kind).upper()
    if model_key == 'AUTO':
        ...
        if XGBRegressor is None:
            raise ImportError("Requested model_kind='XGB' but xgboost is not installed.")
        model = XGBRegressor(n_estimators=200, max_depth=5, random_state=42, verbosity=0)
    elif model_key == 'RF':
        model = RandomForestRegressor(n_estimators=400, max_depth=12, random_state=42)
    elif model_key == 'GB':
...
def create_styled_heatmap_pipeline(df, target_col='y1',
    ...
    model, data = train_best_model(df, target_col, model_kind=model_kind)
