...
from src.output_paths import output_dir, output_file
...
def main():
    ...
    figure_dir = str(output_dir(project_root, 'extended', 'figures'))
    report_path = str(output_file(project_root, 'extended', 'reports', 'extended_results_summary.txt'))
    ...
    plot_regression_comparison_extended(
        reg_results,
        save_path=os.path.join(figure_dir, 'extended_regression_comparison.png')
    )
    
    print("[2/4] Classification comparison chart...")
    plot_classification_comparison_extended(
        clf_results,
        save_path=os.path.join(figure_dir, 'extended_classification_comparison.png')
    )
    
    print("[3/4] Combined dashboard...")
    plot_combined_dashboard(
        reg_results, clf_results,
        save_path=os.path.join(figure_dir, 'extended_dashboard.png')
    )
    
    print("[4/4] Ranking heatmap...")
    plot_model_ranking_heatmap(
        reg_results, clf_results,
        save_path=os.path.join(figure_dir, 'extended_ranking_heatmap.png')
    )
