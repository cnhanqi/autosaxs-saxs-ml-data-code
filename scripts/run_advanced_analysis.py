...
def main():
    ...
    figure_dir = str(output_dir(project_root, 'advanced', 'figures'))
    report_path = str(output_file(project_root, 'advanced', 'reports', 'advanced_analysis_report.txt'))
    ...
    analyzer.plot_comprehensive_evaluation(figure_dir)
    
    # SHAP Analysis
    if HAS_SHAP:
        # Find best model for SHAP
        best_model_name = results.loc[results['AUC'].idxmax(), 'Model']
        analyzer.run_shap_analysis(model_name=best_model_name, save_dir=figure_dir)
    else:
        print("\nSHAP not installed. Skipping interpretability analysis.")
        print("Install with: pip install shap")
    
    # Save report
    analyzer.save_comprehensive_report(report_path)
